...
<div class="all-orders mt-2">
  <div class="page-titre">    <h2>Orders</h2><hr></div>
  <table class="table table-bordered table-hover">
    <thead>
      <tr>
        ...
        <th scope="col" class="bg-primary text-white">Valider</th>
    ...
    <tbody>
      <tr>
        ...
        <td>
          <form action="{{Route('dashboard.orders.valide',$key->id)}}" method="post" id="inline_form">
              @csrf
              <button type="submit" class="btn btn-outline-success"><i class="fas fa-check"></i></button>
          </form>
      </td>
        <td>
            <form action="{{Route('dashboard.orders.delete',$key->id)}}" method="post" id="inline_form">
                @csrf
                @method('delete')
